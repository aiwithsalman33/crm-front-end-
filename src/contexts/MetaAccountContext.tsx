...
interface MetaAccountContextType {
  ...
  connectAccount: (accessToken: string, provider?: 'meta' | 'google') => void;
...
export const MetaAccountProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshAdAccountsInternal = async (token: string) => {
    try {
      if (token.startsWith('DEMO_TOKEN')) return;
  ...
  const connectAccount = async (accessToken: string, provider: 'meta' | 'google' = 'meta') => {
    setIsLoading(true);
    setError(null);

    try {
      if (accessToken.startsWith('DEMO_TOKEN')) {
        ...
        const account: MetaAccount = {
          id: isGoogle ? 'google_demo_id' : 'meta_demo_123',
          email: isGoogle ? 'demo@google-ads.com' : 'demo@meta-ads.com',
          name: isGoogle ? 'Google Ads Demo User' : 'Meta Demo User',
          accessToken,
          adAccounts: [{
            id: 'act_123456789',
            name: isGoogle ? 'Google Ads Demo Account' : 'Meta Ads Demo Account',
            accountId: '123456789',
            currency: 'USD',
  ...
  const refreshAdAccounts = async (): Promise<void> => {
    ...
    if (metaAccount.accessToken.startsWith('DEMO_TOKEN')) {
      return; // No need to refresh for demo
    }
  ...
  const fetchCampaigns = async (adAccountId: string): Promise<MetaCampaign[]> => {
    ...
    if (metaAccount.accessToken.startsWith('DEMO_TOKEN')) {
      ...
      // Dynamically import to avoid circular dependency issues during initialization if possible, 
      // or just rely on the updated utils being available.
      // We will assume utils functions are available or we can duplicate basic mock logic here 
      // BUT better to import. Let's add the import to the top of the file.
      const { generateMockCampaigns } = require('../features/adsSync/utils');
    ...
    try {
      ...
      return data.data.map((campaign: any) => {
        ...
        return {
          ...
          leads_count: parseInt(insights.conversions) || 0
        };
      });
    } catch (err) {
  ...
  const fetchAdSets = async (campaignId: string): Promise<MetaAdSet[]> => {
    ...
    if (metaAccount.accessToken.startsWith('DEMO_TOKEN')) {
      ...
      const { generateMockAdSets } = require('../features/adsSync/utils');
      return generateMockAdSets(campaignId);
    }
  ...
  const fetchAds = async (campaignId: string): Promise<MetaAd[]> => {
    ...
    if (metaAccount.accessToken.startsWith('DEMO_TOKEN')) {
      await new Promise(resolve => setTimeout(resolve, 400));
      const { generateMockAds } = require('../features/adsSync/utils');
      return generateMockAds(campaignId);
    }
